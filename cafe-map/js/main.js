(function($) {
    ...
    function maps() {
        ...
        $('.cart-cafe').each(function() {
            ...
            let $thisLat = $address.data('lat');
            let $thisLon = $address.data('lon');

            if ( $thisLat && $thisLon && 0 !== $thisLat && 0 !== $thisLon ) {
                data.push( {'lat':$thisLat, 'lon':$thisLon, 'title': $thisName, 'html': '<h5>' + $thisName + '</h5>', 'zoom': 15,} );
            }
        });
